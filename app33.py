...
# 2. 고정 설정
CAPA_INFO = {"조립1": 3300, "조립2": 3700, "조립3": 36089}
CAPA_90_PERCENT = {"조립1": 2970, "조립2": 3330, "조립3": 32480}
...
**해결**: 조립2 → 조립1로 500개 이동 (4일 후인 10월 19일로), 달성률 98.5%
    ...
    if not current_df.empty:
        ...
        for line in sorted(all_products_by_line.keys()):
            ...
            for prod in products:
                movement_rules += f"  - {prod}\n"
        
        movement_rules += """
⚠️ **중요 규칙: 품목 라인 이동 제약**
1. 품목을 다른 라인으로 이동하려면, **목적지 라인에 해당 품목이 존재**해야 합니다
2. 예: "조립1의 A품목 → 조립2"는 조립2에 A품목이 있을 때만 가능
3. qty_0차가 0이어도 품목 행이 존재하면 이동 가능
4. **위 목록에 없는 품목으로는 절대 이동 제안 금지**

📅 **날짜 이동 규칙 (중요!)**
5. **라인 간 이동 시 반드시 4일 후로 이동해야 합니다**
6. 예: 1월 5일 조립1 → 조립2로 이동 = 1월 9일 조립2로 배치
7. 같은 라인 내 날짜 변경은 자유롭게 가능
8. 다른 날짜로 이동 가능하며, 라인 변경 시에만 +4일 적용

✅ **이동 가능 예시:**
- 1/5 조립1 "FAN_V710 1200개" → 1/9 조립2 (조립2에 FAN_V710 있음) ✅
- 1/5 조립1 "FAN_V710 1200개" → 1/7 조립1 (같은 라인) ✅
- 1/10 조립2 "FLANGE_A 500개" → 1/14 조립3 (조립3에 FLANGE_A 있음) ✅

❌ **이동 불가능 예시:**
- 1/5 조립1 "FAN_V710" → 1/6 조립2 (4일 후가 아님) ❌
- 1/5 조립1 "FAN_V710" → 1/9 조립3 (조립3에 FAN_V710 없음) ❌
- 조립1에만 있는 품목을 조립2/3으로 이동 ❌
"""
...
4. 라인 간 이동 시 반드시 +4일 계산 (예: 1/5 → 1/9)
"""
    ...
    if st.session_state.target_date:
        ...
        if not debug_plan.empty:
            ...
            for line in sorted(debug_plan['line'].unique()):
                ...
                st.write(f"**{line}** ({len(products)}개): {', '.join(products[:10])}{'...' if len(products) > 10 else ''}")
    else:
